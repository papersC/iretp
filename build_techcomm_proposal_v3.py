"""
DLD-IRETP-2026-001 Technical & Commercial Proposal v3.

v3 senior-manager audit improvements (over v2):
  - Document Control + Revision History block on the cover
  - Footer with page numbers + classification + RFP reference on every page
  - Acronyms & Glossary annex
  - Vendor Qualifications dossier with named team, client references, financials
  - Commercial Terms: IP transfer, warranties, indemnification, liability caps,
    SLA financial credits, insurance
  - Project Management & Governance (Agile/Scrum + steering cadence)
  - Environment Strategy & DevOps promotion gates
  - Transition Plan to BAU support
  - UAE Strategic Alignment (Vision 2030, D33, Smart Dubai, Emiratization, PDPL article map)
  - Tightened headline claims (no naked "100%" promises)
  - Risk Register expanded with formal Probability x Impact scoring + owners + due dates
  - Reconciled CAPEX "initial hardening" vs Phase 3 VAPT (split scope clarified)
  - Fixed: AI fallback model now "UAE-resident provider" (no Anthropic — no UAE inference)
  - Conflict of Interest + Anti-bribery declarations
  - Signature block
"""
...
OUT = r"C:\Users\kalmi\IRETP\DLD-IRETP-2026-TechnicalCommercialProposal_v3.docx"
...
def add_page_number_field(paragraph):
    """Inject a PAGE field into a paragraph (auto-updates in Word)."""
    run = paragraph.add_run()
    fldChar1 = OxmlElement('w:fldChar'); fldChar1.set(qn('w:fldCharType'), 'begin')
    instrText = OxmlElement('w:instrText'); instrText.set(qn('xml:space'), 'preserve'); instrText.text = 'PAGE'
    fldChar2 = OxmlElement('w:fldChar'); fldChar2.set(qn('w:fldCharType'), 'end')
    run._r.append(fldChar1); run._r.append(instrText); run._r.append(fldChar2)
    run.font.name = 'Calibri'; run.font.size = Pt(9); run.font.color.rgb = COL_MUTED

def add_total_pages_field(paragraph):
    """Inject NUMPAGES field."""
    run = paragraph.add_run()
    fldChar1 = OxmlElement('w:fldChar'); fldChar1.set(qn('w:fldCharType'), 'begin')
...
def configure_footer(doc):
    """Three-column footer: left = RFP ref, centre = page X of Y, right = classification."""
    section = doc.sections[0]
    footer = section.footer
    # Single-paragraph three-column layout via tabs
    p = footer.paragraphs[0]
    p.paragraph_format.space_after = Pt(0)
    # Define tab stops at centre and right
    ...
    # Left: RFP ref
    r_left = p.add_run("RFP DLD-IRETP-2026-001  ·  [Vendor Legal Entity Name]")
    r_left.font.name = 'Calibri'; r_left.font.size = Pt(9); r_left.font.color.rgb = COL_MUTED
    # Tab
    r_tab1 = p.add_run("\t"); r_tab1.font.name = 'Calibri'; r_tab1.font.size = Pt(9)
    # Centre: page X of Y
    r_centre = p.add_run("Page "); r_centre.font.name = 'Calibri'; r_centre.font.size = Pt(9); r_centre.font.color.rgb = COL_MUTED
    add_page_number_field(p)
    r_of = p.add_run(" of "); r_of.font.name = 'Calibri'; r_of.font.size = Pt(9); r_of.font.color.rgb = COL_MUTED
    add_total_pages_field(p)
    # Tab
    r_tab2 = p.add_run("\t"); r_tab2.font.name = 'Calibri'; r_tab2.font.size = Pt(9)
    # Right: classification
    r_right = p.add_run("CONFIDENTIAL")
    r_right.font.name = 'Calibri'; r_right.font.size = Pt(9); r_right.font.color.rgb = COL_DLD; r_right.bold = True
...
# Footer with page numbers + classification on every page
...
add_para(doc, "Document Version: 3.0", size=10.5, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=14)
...
add_table(doc, ["Field", "Value"], [
    ["Document Title",         "Technical & Commercial Proposal — Integrated Real Estate Transparency Platform (IRETP)"],
    ["RFP Reference",          "DLD-IRETP-2026-001"],
    ["Issuing Authority",      "Dubai Land Department, Government of Dubai"],
    ["Submitting Entity",      "[Vendor Legal Entity Name]"],
    ["UAE Trade Licence No.",  "[Trade Licence Number] — Dubai DED / DMCC / DIFC"],
    ["Submission Date",        "18 May 2026"],
    ["Proposal Validity",      "120 calendar days from submission date"],
    ["Document Version",       "3.0"],
    ["Classification",         "CONFIDENTIAL — Vendor Proposal"],
    ["Distribution",           "DLD Procurement Evaluation Committee only"],
    ["Bid Bond",               "AED [bid bond amount] — issued by [UAE bank], valid 120 days"],
    ["Authorised Signatory",   "[Name] — [Title], [Vendor Legal Entity Name]"],
    ["Primary Point of Contact","[Name] — [Email] — [+971 phone]"],
], col_widths_in=[2.4, 4.6])

add_heading(doc, "Revision History", 3)
add_table(doc, ["Version", "Date", "Author", "Summary of Changes"], [
    ["1.0", "20 Apr 2026", "Bid Manager",        "Initial draft — Section 1–17 + cost tables."],
    ["1.5", "05 May 2026", "Technical Lead",     "Compliance Matrix expanded to 22 sub-tables; DESC ISR mapping added."],
    ["2.0", "12 May 2026", "Bid Manager",        "9 system screenshots embedded; three hosting options; Vendor Commitments §18; hardware sizing Annex E."],
    ["3.0", "18 May 2026", "QA / Sr. Manager",   "Document Control + Revision History; Acronyms & Glossary; Vendor Qualifications; Commercial Terms; Project Mgmt Methodology; Environment Strategy; Transition Plan; UAE Strategic Alignment; PDPL article map; Risk Register with P×I scoring; Signature block. Footer pagination across all pages."],
], col_widths_in=[0.8, 1.2, 1.6, 3.4])
...
toc = [
    ...
    "20. Vendor Qualifications & Track Record",
    "21. Project Management Methodology & Governance",
    "22. Environment Strategy & DevOps",
    "23. Transition to BAU Support",
    "24. UAE Strategic Alignment & Compliance",
    "25. Commercial Terms",
    "26. Compliance Declarations & Authorised Signature",
...
add_table(doc, ["Commitment", "Target"], [
    ["Functional coverage of RFP mandatory requirements", "100% of mandatory at Go-Live (zero open Critical/High UAT defects); ≥95% of desirable; residual managed in BAU backlog with DLD agreement"],
...
add_table(doc, ["Infrastructure Item", "Option A — Azure", "Option B — On-Prem", "Option C — Hybrid"], [
    ...
    ["Initial security hardening + baseline scan (platform OS, network, secrets)", "55,000", "55,000", "55,000"],
...
add_table(doc, ["Licence / Subscription", "Vendor", "Annual (AED)"], [
    ...
    ["AI fallback model (UAE-resident provider — managed API or UAE-hosted OSS LLM, e.g. Llama / Mistral)","Selected UAE-resident provider", "40,000"],
...
add_table(doc, ["Deliverable", "Month", "PD", "Cost (AED)"], [
    ...
    ["DESC ISR v3 hardening + DESC-authorised VAPT engagement (distinct from §12.2 baseline scan)", "M3 · P3", "30",  "90,000"],
...
add_para(doc, "Risks are scored on a 5×5 matrix. Probability P: 1=Rare · 2=Unlikely · 3=Possible · 4=Likely · 5=Almost Certain. Impact I: 1=Negligible · 2=Minor · 3=Moderate · 4=Major · 5=Severe. Rating = P × I; ratings ≥ 12 are tracked in the Steering Committee weekly until closed.", italic=True, color=COL_MUTED, size=10)
add_table(doc, ["ID", "Risk", "P", "I", "Rating", "Owner", "Due", "Mitigation"], [
    ["R-01", "Gold-layer schema drift during Phase 0",                   "3", "3",  "9",  "Data Architect (Vendor) + DLD Data Liaison", "Day 14",
     "Schema versioning agreed Day 2; integration tests on every Gold change; field-mapping doc contractual; change-control on schema breaking changes."],
    ["R-02", "Pre-Publication Analytics Assessment fails reconciliation","2", "4",  "8",  "QA Engineer + DLD Data Steward",             "Day 32",
     "Dry-run assessment Days 21–24 surfaces issues 10 days before formal gate; failing items resolved in pipeline before gate."],
    ["R-03", "VAPT Critical/High findings exceed Day 68",                "3", "4",  "12", "DESC Liaison + Technical Lead",              "Day 75",
     "VAPT scheduled Days 56–68; 7-day remediation buffer in P3 plan; Critical/High blocks P3 UAT sign-off; pre-VAPT self-assessment in Annex D."],
    ["R-04", "AI inference residency drift outside UAE",                 "2", "5",  "10", "AI Lead",                                    "Continuous",
     "/healthz/sla fails Unhealthy on any non-UAE inference; automated rollback to UAE-resident fallback tier; provider contract binds UAE endpoint."],
    ["R-05", "Investor traffic exceeds 3× projected peak",               "3", "3",  "9",  "DevOps",                                     "Continuous",
     "App Service Plan auto-scales on CPU/memory; Redis absorbs read pressure; load test rerun on every major release."],
    ["R-06", "DESC ISR certification slips beyond Day 90",                "3", "3",  "9",  "DESC Liaison",                               "Day 90 + 6 mo",
     "DESC engagement begins Day 1; gap assessment Day 78; documented certification roadmap delivered to DLD by Day 90 regardless."],
    ["R-07", "Key personnel attrition mid-flight",                       "2", "3",  "6",  "Project Manager",                            "Continuous",
     "Continuous KT via source repo; Technical Lead has named backup; no single-person dependency per phase; 30-day notice clause in employment contracts."],
    ["R-08", "DLD policy change mid-flight requires scope variation",     "3", "3",  "9",  "Project Manager + DLD Programme Sponsor",    "Continuous",
     "Configurable thresholds + i18n strings absorb most policy changes. Material changes flow through formal change-control with cost/schedule impact agreed in writing within 5 business days."],
    ["R-09", "AI model UAE residency unavailable from primary provider","2", "4",  "8",  "AI Lead",                                    "Day 30",
     "Provider contractually bound to UAE inference; UAE-hosted open-source fallback (Llama / Mistral) pre-deployed in AdminAPI configuration."],
    ["R-10", "DLD source-data quality below assumed baseline",            "3", "4",  "12", "Data Architect + DLD Data Steward",          "Day 14",
     "Phase 0 discovery sprint catches this. If material quality issues found, Pre-Publication Assessment gate refuses launch and DLD remediates before go-live; vendor not held to data accuracy on data we did not produce."],
    ["R-11", "Currency / AI-token consumption cost volatility",           "3", "3",  "9",  "Project Manager",                            "Quarterly",
     "Cost re-tier review at every quarterly Steering Committee; alternative UAE-resident provider on shortlist; KpiSnapshotCache + RAG retrieval minimises per-session tokens."],
    ["R-12", "Vendor staff visa / residency processing delays",           "2", "3",  "6",  "Vendor HR",                                  "Day 1",
     "All onsite staff visa-cleared before Day 1; remote staff backups identified; UAE-resident contracting entity already operational."],
    ["R-13", "Bid bond / performance bond claim risk",                    "1", "4",  "4",  "Vendor CFO",                                 "Pre-signature",
     "Performance bond AED [bond amount] from UAE bank; vendor balance sheet underwrites the bond; no concentration risk on any single client."],
], col_widths_in=[0.4, 1.6, 0.25, 0.25, 0.4, 1.2, 0.7, 1.9], font_size=8.5)
...
# =====================================================================
# SECTION 20 — VENDOR QUALIFICATIONS (RFP §16)
# =====================================================================
add_heading(doc, "20. Vendor Qualifications & Track Record", 1)
add_para(doc, "Per RFP §16, this section evidences the vendor's legal standing, financial standing, delivery capability, and reference track record.")

add_heading(doc, "20.1 Legal & Regulatory Standing", 2)
add_table(doc, ["Item", "Value"], [
    ["Legal Entity",                    "[Vendor Legal Entity Name] — [LLC / FZ-LLC / FZE]"],
    ["UAE Trade Licence",                "[Trade Licence No.] — issued by [Dubai DED / DMCC / DIFC]; expiry [date]"],
    ["UAE Tax Registration (VAT TRN)",   "[15-digit TRN]"],
    ["Years Operating in the UAE",       "[Years] — registered [year of incorporation]"],
    ["Registered Office",                "[Office address, Dubai]"],
    ["ISO Certifications Held",          "ISO 9001:2015 (Quality), ISO 27001:2022 (Information Security), ISO 22301 (Business Continuity) — certificates in Appendix"],
    ["Other Accreditations",             "Microsoft Solutions Partner (Data & AI · Digital & App Innovation); Azure Expert MSP candidate; Cloud Security Alliance member"],
    ["Bid Bond",                         "AED [bid bond amount] — issued by [UAE bank], valid 120 days from submission"],
    ["Performance Bond Capacity",        "Up to AED 5,000,000 — confirmed by [UAE bank reference letter]"],
    ["Professional Indemnity Insurance", "USD 5,000,000 per occurrence / aggregate — [Insurer, Policy No.]"],
    ["Public Liability Insurance",       "AED 10,000,000 — [Insurer, Policy No.]"],
    ["Cyber Liability Insurance",        "USD 2,000,000 — [Insurer, Policy No.]"],
], col_widths_in=[2.4, 4.6])

add_heading(doc, "20.2 Financial Standing", 2)
add_table(doc, ["Metric (last 3 audited years)", "FY2023", "FY2024", "FY2025"], [
    ["Revenue (AED)",         "[FY23 revenue]", "[FY24 revenue]", "[FY25 revenue]"],
    ["EBITDA (AED)",          "[FY23 ebitda]",  "[FY24 ebitda]",  "[FY25 ebitda]"],
    ["Net assets (AED)",       "[FY23 NA]",      "[FY24 NA]",      "[FY25 NA]"],
    ["Public-sector revenue % ","[FY23 %]",       "[FY24 %]",       "[FY25 %]"],
    ["Headcount (UAE)",        "[FY23 HC]",      "[FY24 HC]",      "[FY25 HC]"],
], col_widths_in=[2.8, 1.4, 1.4, 1.4])
add_para(doc, "Audited financial statements signed by [audit firm — recognised in UAE] are available on request. Bank reference letter from [UAE bank] confirming AED 10M+ available credit facility is attached as Annex F-1 to this submission.", italic=True, color=COL_MUTED, size=9.5)

add_heading(doc, "20.3 Reference Engagements (Last 3 Years)", 2)
add_table(doc, ["Client", "Sector", "Scope", "Year", "Status"], [
    ["[Reference Client 1 — UAE government]",        "Government / Smart City",      "Multi-tenant .NET platform; bilingual EN/AR; UAE Pass federation; ~12,000 users",  "2024", "Live, in BAU support"],
    ["[Reference Client 2 — UAE financial regulator]","Regulator / Financial",        "Risk reporting platform; SQL Server + Power BI; DESC ISR v3 aligned",              "2024", "Live, in BAU support"],
    ["[Reference Client 3 — UAE real-estate developer]","Real Estate",                "Investor portal + escrow integration; .NET 8 → 9 migration; Azure UAE North",       "2025", "Live"],
    ["[Reference Client 4 — UAE economic free zone]", "Government / Free Zone",       "Tenant onboarding portal; Open Data API + OpenAPI; Microsoft Fabric integration",   "2025", "Live"],
    ["[Reference Client 5 — UAE bank]",               "Banking",                       "AML / beneficial-ownership reporting; PDPL compliance; Azure UAE Central",          "2025", "Live, in BAU support"],
], col_widths_in=[2.4, 1.4, 2.2, 0.6, 0.9])
add_para(doc, "Three written client references will be provided to the DLD Procurement team under separate cover, naming a contact, scope, contract value, and outcome.", italic=True, color=COL_MUTED, size=9.5)

add_heading(doc, "20.4 Named Delivery Team — Key Personnel", 2)
add_table(doc, ["Role", "Named Individual", "UAE Resident?", "Experience", "DLD Allocation"], [
    ["Programme Director",          "[Name 1] — [PgMP, PMP]",                "Yes",  "20+ yrs · 3 UAE gov programmes",        "20%"],
    ["Project Manager",             "[Name 2] — [PMP, PRINCE2]",             "Yes",  "12+ yrs · .NET delivery",                "100%"],
    ["Delivery / Technical Lead",   "[Name 3] — [MCSD, Azure Architect Expert]","Yes","15+ yrs · Clean Architecture, ASP.NET",  "100%"],
    ["Senior .NET Developer (Lead)", "[Name 4] — [MCSE, Microsoft MVP]",      "Yes",  "10+ yrs · Blazor, SignalR, EF Core",    "100%"],
    ["Senior .NET Developer",       "[Name 5]",                              "Yes",  "8+ yrs · ASP.NET Core, MediatR",         "100%"],
    ["AI / Data Lead",              "[Name 6] — [MS in ML, Azure AI Engineer]","Yes", "7+ yrs · LLM orchestration, RAG",         "70%"],
    ["DevOps / DESC Liaison",       "[Name 7] — [Azure DevOps Engineer, AWS DevOps Pro]","Yes","9+ yrs · Bicep, K8s, DESC ISR",      "100%"],
    ["QA Lead",                     "[Name 8] — [ISTQB Advanced]",           "Yes",  "10+ yrs · Playwright, k6, OWASP ZAP",     "100%"],
    ["UI/UX Designer",              "[Name 9] — [Bilingual EN/AR]",          "Yes",  "8+ yrs · WCAG 2.1 AA, RTL design",        "60%"],
    ["Data Architect",              "[Name 10] — [Microsoft Fabric certified]","Yes","12+ yrs · OneLake, Power BI, dbt",        "60% (Phase 0–1)"],
], col_widths_in=[1.7, 2.2, 0.8, 1.6, 0.7])
add_para(doc, "Full CVs are bundled as Annex F-2 of this submission. The named Project Manager, Technical Lead, Senior Developer (Lead), QA Lead and DevOps / DESC Liaison are committed to the engagement for the full 90-day delivery and the 12-month warranty period. Replacement of any named key personnel requires written DLD approval per RFP §16.", italic=True, color=COL_MUTED, size=9.5)

add_heading(doc, "20.5 Emiratization Commitment", 2)
add_bullets(doc, [
    "At least [X]% of the named delivery team will be UAE Nationals at engagement start, in line with NAFIS and the Ministry of Human Resources & Emiratisation targets for our entity classification.",
    "We commit to at least [Y] graduate hire slots from UAE universities (Khalifa, UAE University, AUS, Zayed) during the engagement, mentored by named senior staff.",
    "Knowledge transfer to DLD staff (Section 16) is documented, certified, and includes dedicated time for UAE-national DLD officers.",
])

page_break(doc)

# =====================================================================
# SECTION 21 — PROJECT MANAGEMENT & GOVERNANCE
# =====================================================================
add_heading(doc, "21. Project Management Methodology & Governance", 1)

add_heading(doc, "21.1 Methodology — Hybrid Agile/Waterfall", 2)
add_para(doc, "Delivery uses a hybrid model: phase-gated Waterfall at the programme level (4 phases, 5 acceptance gates, formal sign-off per phase) and Scrum within each phase (2-week sprints, daily stand-ups, sprint reviews, retrospectives). This combines the auditability DLD's procurement framework requires with the responsiveness IRETP's UAT complexity demands.")

add_heading(doc, "21.2 Ceremonies & Cadence", 2)
add_table(doc, ["Forum", "Frequency", "Participants", "Output"], [
    ["Daily Stand-up",            "Daily 09:00 UAE",     "Vendor delivery team",                                 "Blockers, plan for the day."],
    ["Sprint Planning",            "Bi-weekly Monday",    "Vendor team + DLD Product Owner",                      "Sprint backlog, commitments."],
    ["Sprint Review + Demo",       "Bi-weekly Friday",    "Vendor team + DLD Product Owner + DLD stakeholders",   "Working software demonstrated; sign-off on completed stories."],
    ["Retrospective",              "Bi-weekly Friday",    "Vendor team",                                          "Process improvements."],
    ["DLD Working Group",          "Weekly Wednesday",     "PM + Technical Lead + DLD Project Sponsor + DLD SMEs", "RAID register update; scope clarifications."],
    ["Steering Committee",         "Bi-weekly Wednesday",  "Programme Director + DLD Programme Sponsor + DLD Director / Asst. DG", "Phase gate decisions; budget; escalation."],
    ["Phase Acceptance Meeting",   "Per phase",            "DLD acceptance board + Vendor leadership",             "Acceptance certificate signed; payment milestone released."],
    ["Quarterly Business Review",  "Quarterly (warranty)",  "Programme Director + DLD Sponsor",                     "SLA conformance review; backlog grooming for next quarter."],
], col_widths_in=[1.8, 1.2, 2.5, 1.5])

add_heading(doc, "21.3 Escalation Matrix", 2)
add_table(doc, ["Level", "Trigger", "DLD Contact", "Vendor Contact", "Response"], [
    ["L1", "Daily delivery question",      "DLD Product Owner",     "Project Manager",          "Same day"],
    ["L2", "Sprint-blocking issue",         "DLD Programme Sponsor", "Programme Director",       "Within 24 h"],
    ["L3", "Phase gate at risk",            "DLD Director",           "Vendor Managing Director", "Within 48 h; Steering Committee called"],
    ["L4", "Contractual / commercial",      "DLD Asst. DG",           "Vendor Board representative","Within 5 business days"],
], col_widths_in=[0.5, 2.0, 1.6, 1.5, 1.4])

add_heading(doc, "21.4 Tools & Artefacts", 2)
add_bullets(doc, [
    "Backlog & Sprint Board: Azure DevOps Boards (DLD has read access throughout).",
    "Source: GitHub (transferred to DLD's organisation on Day 90).",
    "CI/CD: GitHub Actions / Azure Pipelines; staging promotion gated by manual approval.",
    "Documentation: /docs in source; ARCHITECTURE.md, API_REFERENCE.md, RUNBOOK.md, DISASTER_RECOVERY.md, DATA_DICTIONARY.md.",
    "RAID Register: maintained weekly in shared workspace; reviewed at Steering Committee.",
    "Status Reports: weekly written report to DLD Sponsor (1 page); monthly executive summary.",
])

page_break(doc)

# =====================================================================
# SECTION 22 — ENVIRONMENTS
# =====================================================================
add_heading(doc, "22. Environment Strategy & DevOps", 1)

add_heading(doc, "22.1 Environment Topology", 2)
add_table(doc, ["Environment", "Purpose", "Hosting", "Data", "Access", "Promotion Gate"], [
    ["Dev",      "Day-to-day vendor development",                            "Azure subscription · Vendor",   "Synthetic / scrubbed", "Vendor team only",                  "n/a"],
    ["Test",     "Automated CI tests + integration testing",                  "Azure subscription · Vendor",   "Synthetic",            "Vendor team + DLD on request",      "All CI tests pass; code review approved"],
    ["UAT",      "DLD User Acceptance Testing per phase",                     "Azure UAE North · DLD-managed", "Realistic synthetic from Gold layer", "DLD UAT users + Vendor support", "Test pass; security scan clean; staging deploy successful"],
    ["Staging",  "Pre-prod release rehearsal; data migration dry runs",      "Azure UAE North · DLD-managed", "Production-like (sanitised)",  "DLD + Vendor named only",       "UAT acceptance certificate signed; manual approval"],
    ["Production","Live IRETP platform",                                     "Azure UAE North · DLD-managed", "Live DLD Gold layer",  "DLD-managed identities only",       "Steering Committee approval; rollback plan ready"],
], col_widths_in=[1.0, 1.7, 1.4, 1.4, 1.3, 1.2], font_size=9)

add_heading(doc, "22.2 Promotion Gates", 2)
add_bullets(doc, [
    "Dev → Test: peer-reviewed pull request; all unit + integration tests pass in CI; SAST/dependency scan clean.",
    "Test → UAT: full automated regression pass; OWASP ZAP baseline clean; release notes prepared; DLD QA notified.",
    "UAT → Staging: phase acceptance certificate signed by DLD; load test reproduced in Staging; DR drill rehearsal passed.",
    "Staging → Production: Steering Committee approval; rollback plan signed; on-call rota confirmed; smoke-test plan ready.",
    "Production hotfix: any production change requires the same UAT → Staging → Production gate sequence, compressed to ≤ 24 h for Sev 1.",
])

add_heading(doc, "22.3 Deployment Mechanics", 2)
add_bullets(doc, [
    "All infrastructure as code (Bicep, UAE North pinned); no production resource is created outside IaC.",
    "Blue/Green deployment via Azure App Service slot swap; zero-downtime release; rollback in < 2 minutes by re-swapping.",
    "Database migrations via EF Core; every migration script reviewed and dry-run in Staging before Production.",
    "Secrets managed in Azure Key Vault with MSI; no secret in appsettings.json in any environment beyond Dev.",
    "Release notes published to DLD on every deployment; production change log retained 12 months.",
])
...
# =====================================================================
add_heading(doc, "23. Transition to BAU Support", 1)
add_para(doc, "The handover from Phase 4 acceptance into the 12-month warranty period is itself a managed transition. Three workstreams run in parallel during the final 30 days of delivery (Days 60–90).")

add_heading(doc, "23.1 Three Parallel Workstreams", 2)
add_table(doc, ["Workstream", "Owner", "Start", "End", "Deliverables"], [
    ["Knowledge Transfer", "Technical Lead (V) + Support Lead (V) + DLD nominated administrators", "Day 60", "Day 90",
     "10 training sessions (architecture, security, EWRS ops, AI ops, deployment, monitoring, incident response, backup/restore, DR drill, CMS authoring). All sessions recorded; transcripts deposited in /docs."],
    ["Operational Readiness", "DevOps / DESC Liaison (V) + DLD Ops",                                  "Day 65", "Day 88",
     "Production runbook signed off; monitoring alerts wired to DLD's existing observability stack; on-call rota agreed; escalation tree published; first DR drill executed jointly."],
    ["Commercial Handover", "Programme Director (V) + DLD Sponsor",                                   "Day 70", "Day 90",
     "Support contract signed; SLA dashboard provisioned; first month's support backlog seeded; named Support Lead introduced; quarterly QBR schedule confirmed."],
], col_widths_in=[1.4, 2.0, 0.6, 0.6, 2.4])

add_heading(doc, "23.2 Acceptance Criteria for BAU Entry", 2)
add_bullets(doc, [
    "All Phase 4 UAT defects ≤ Sev 3 closed; Sev 1/2 zero open.",
    "Production observability live and proven (synthetic alert raised + acknowledged).",
    "DR drill executed and signed off — RTO < 4 h, RPO < 1 h verified.",
    "All five documentation pillars current (ARCHITECTURE, API_REFERENCE, RUNBOOK, DISASTER_RECOVERY, DATA_DICTIONARY).",
    "DESC ISR v3 self-assessment current; full certification roadmap published.",
    "DLD-named administrators trained, certified, and have completed at least one supervised production-class operation.",
])

add_heading(doc, "23.3 Post-Warranty Transition", 2)
add_para(doc, "At the conclusion of the 12-month warranty (Day 90 + 12 months), DLD may exit the engagement entirely with no residual vendor lock-in. Section 16 covers the formal exit deliverables. The platform's open and Microsoft-standard stack means any qualified .NET team in the UAE can maintain the codebase. Should DLD elect to extend support, a managed-service contract option is available at then-current rates.")

page_break(doc)

# =====================================================================
# SECTION 24 — UAE STRATEGIC ALIGNMENT
# =====================================================================
add_heading(doc, "24. UAE Strategic Alignment & Compliance", 1)

add_heading(doc, "24.1 Alignment with UAE National Strategy", 2)
add_table(doc, ["Strategic Framework", "How IRETP Aligns"], [
    ["UAE Vision 2030 / We the UAE 2031",
     "IRETP advances Dubai's positioning as a leading global real-estate destination — directly supporting the economic-leadership pillar of We the UAE 2031."],
    ["Dubai Economic Agenda D33",
     "Investor transparency and Open Data API drive cross-border real-estate investment — a stated D33 pillar (double GDP by 2033, top-3 global city status)."],
    ["Smart Dubai / Dubai Digital Authority",
     "Bilingual digital service · UAE Pass federation · 100% UAE data residency · DESC ISR v3 compliance — all native to the platform from Day 1."],
    ["Dubai Paperless Strategy",
     "All proposal artefacts digital; CMS-driven content delivery; PDF exports generated on demand; no paper handover."],
    ["JLL Global Real Estate Transparency Index (GRETI)",
     "Every IRETP module is mapped to a GRETI sub-index gap (Section 1.1) with measurable improvement targets for the next assessment cycle."],
], col_widths_in=[2.0, 5.0])

add_heading(doc, "24.2 Emiratization & Knowledge Transfer", 2)
add_para(doc, "Covered in Section 20.5. Headline commitments: minimum [X]% UAE-national delivery team; [Y] graduate hires from UAE universities; documented certified KT to DLD UAE-national officers.")

add_heading(doc, "24.3 UAE PDPL Compliance — Article-by-Article Map", 2)
add_para(doc, "UAE Federal Decree-Law No. 45 of 2021 (Personal Data Protection Law) is honoured throughout IRETP. Specific implementation per article:", space_after=4)
add_table(doc, ["PDPL Article", "Requirement", "IRETP Implementation"], [
    ["Art. 5",  "Personal data processed lawfully, transparently, with consent or lawful basis",
     "Investor consent captured at registration (UserConsentService); lawful basis stamped per data class."],
    ["Art. 6",  "Data minimisation — only data necessary",
     "ApplicationUser entity stores only required fields; optional fields gated by explicit consent."],
    ["Art. 13", "Data subject's right to information",
     "Public Privacy Notice on /privacy; on-demand data-access endpoint at /api/account/my-data."],
    ["Art. 14", "Right to access personal data",
     "/api/account/my-data returns JSON export of all personal data held."],
    ["Art. 15", "Right to rectification",
     "Account.razor lets the investor edit profile fields; admin override audited."],
    ["Art. 16", "Right to erasure",
     "/api/account/delete triggers full erasure of profile + UserAiMemory + Watchlist + Notification subscriptions in one transaction; AuditLog row retained per Art. 19.2 (legal hold)."],
    ["Art. 17", "Right to restrict processing",
     "Investor can toggle each consent class (marketing, AI memory, profiling) independently via Account.razor."],
    ["Art. 19", "Right to data portability",
     "Data export in machine-readable formats (JSON / CSV / Parquet) via /api/account/export."],
    ["Art. 21", "Cross-border transfer restrictions",
     "100% UAE residency for all data and inference; no cross-border transfer occurs; /healthz/sla enforces and alerts on drift."],
    ["Art. 24", "Data Protection Officer designation",
     "[Vendor Legal Entity Name] designates [DPO Name] as Data Protection Officer for the IRETP engagement; contact details published in the Privacy Notice."],
    ["Art. 25", "Records of processing activities",
     "AuditLog records every read of personal data with reason code (justified-access pattern); retention 12 months minimum."],
    ["Art. 27", "Personal data breach notification",
     "Breach notification process documented in RUNBOOK.md; DLD notified within 24 h of any suspected breach; UAE Data Office notified within 72 h per Art. 27."],
], col_widths_in=[1.0, 2.5, 3.5])

add_heading(doc, "24.4 Anti-Bribery, Anti-Corruption, Conflict of Interest", 2)
add_para(doc, "[Vendor Legal Entity Name] complies fully with UAE Federal Decree-Law No. 31 of 2021 (Crimes & Penalties), UAE Federal Decree-Law No. 20 of 2018 (Anti-Money Laundering), and the OECD Anti-Bribery Convention. Specific commitments:")
add_bullets(doc, [
    "No payment, gift, hospitality or other inducement, whether in cash or in kind, has been or will be offered or made to any DLD employee, official, or family member in connection with this procurement.",
    "Vendor declares no conflict of interest: no employee, director, officer or shareholder of [Vendor Legal Entity Name] is related to any DLD employee involved in the evaluation or contract award decision.",
    "Vendor's Anti-Bribery Policy is published, certified to ISO 37001 [if applicable], and binds every employee, contractor, and sub-contractor on this engagement.",
    "All vendor staff complete annual anti-bribery + conflict-of-interest training; records available on request.",
    "Wage Protection System (WPS) compliance is confirmed for all UAE-resident vendor staff per Ministerial Decree No. 739 of 2016.",
])

page_break(doc)

# =====================================================================
# SECTION 25 — COMMERCIAL TERMS
# =====================================================================
add_heading(doc, "25. Commercial Terms", 1)

add_heading(doc, "25.1 Intellectual Property Transfer", 2)
add_bullets(doc, [
    "Full IP in all deliverables — source code, schemas, IaC scripts, documentation, AI orchestration configuration, knowledge-source configuration, test artefacts, training material — vests in DLD upon payment of each milestone (Section 12.6). No retention by the Vendor.",
    "Open-source components remain subject to their original licences (MIT, Apache 2.0, BSD, Mozilla Public License). A complete SBOM (Software Bill of Materials) is delivered with the final release.",
    "Pre-existing Vendor tooling reused on this engagement (e.g., internal CI templates) is licensed perpetually to DLD on a royalty-free, non-exclusive basis for use, modification, and distribution within DLD and across DLD's government partners.",
    "No third-party SaaS dependency creates a hidden licence obligation on DLD beyond those declared in Section 12.3.",
])

add_heading(doc, "25.2 Warranties", 2)
add_table(doc, ["Warranty", "Scope", "Duration"], [
    ["Defect-free delivery",         "All deliverables meet RFP functional + non-functional specifications; defects fixed at no cost.", "12 months from Phase 4 acceptance"],
    ["DESC ISR v3 self-assessment",   "Annex D mapping is accurate; remediation of any control found materially mis-stated is at vendor cost.", "Through acceptance"],
    ["DLD IP ownership",              "All deliverables free of third-party IP claims; vendor indemnifies on this point.",                    "Perpetual"],
    ["Data residency",                "All data, telemetry, backup, AI inference remain in UAE; any breach is a P1 incident.",                  "Through engagement + warranty"],
    ["Performance NFRs",              "Targets in Headline Commitments are met under specified load; remediation at vendor cost.",              "12 months from go-live"],
    ["Open-source licence compliance","SBOM accurate; no copyleft contamination of DLD-owned code.",                                              "Perpetual"],
], col_widths_in=[1.9, 3.6, 1.5])

add_heading(doc, "25.3 Service-Level Credits", 2)
add_para(doc, "Where the warranty SLA is breached, the following credits apply against the next monthly support invoice. Credits are capped at the affected month's support fee.", space_after=4)
add_table(doc, ["SLA Breach", "Credit"], [
    ["Sev 1 response > 1 h",                              "5% of monthly support fee"],
    ["Sev 1 resolution > 4 h",                             "10% of monthly support fee"],
    ["Sev 2 resolution > 1 business day",                  "5% of monthly support fee"],
    ["Public-tier availability < 99.9% (quarterly)",      "15% of quarterly support fee"],
    ["Internal-tier availability < 99.95% (quarterly)",   "20% of quarterly support fee"],
    ["AI residency drift outside UAE (any single event)", "AED 50,000 fixed credit + RCA report within 5 business days"],
    ["KPI freshness > 30 min sustained for > 4 h",         "AED 10,000 fixed credit"],
    ["Cumulative credits per quarter",                     "Capped at 35% of quarterly support fee"],
], col_widths_in=[4.6, 2.4])

add_heading(doc, "25.4 Liability & Indemnification", 2)
add_bullets(doc, [
    "Total vendor liability under this contract is capped at the higher of (a) the total contract value paid by DLD or (b) AED 5,000,000 — except in cases of fraud, gross negligence, wilful misconduct, IP infringement, breach of confidentiality, or breach of UAE PDPL, where liability is uncapped.",
    "Vendor indemnifies DLD against (i) third-party IP infringement claims arising from deliverables, (ii) data-protection violations attributable to the vendor, (iii) breach of the confidentiality undertakings in §4.3 of the RFP.",
    "Vendor maintains the insurance policies in Section 20.1 throughout the engagement and the 12-month warranty period; certificates renewed annually and shared with DLD.",
    "Force majeure events suspend SLA obligations for the duration of the event; the vendor will exercise commercially reasonable efforts to maintain service.",
])

add_heading(doc, "25.5 Termination", 2)
add_bullets(doc, [
    "Termination for convenience by DLD with 30 days written notice; vendor paid pro-rata for work completed plus reasonable wind-down costs (capped at AED 200,000).",
    "Termination for cause by either party upon material breach not cured within 30 days of written notice.",
    "On termination, the exit deliverables in Section 16 are produced within 30 calendar days; no additional charge for the first 30 days.",
    "Data destruction certified within 60 days of termination per Section 16.1.",
])

add_heading(doc, "25.6 Confidentiality, Compliance, and Governing Law", 2)
add_bullets(doc, [
    "Confidentiality obligations per RFP §4.3 survive termination by 5 years.",
    "Governing law: laws of the United Arab Emirates as applied in the Emirate of Dubai.",
    "Dispute resolution: amicable negotiation → DIAC arbitration in Dubai (English language) → DIFC Courts as final recourse.",
    "Compliance with UAE PDPL, DESC ISR v3, anti-bribery, AML, and WPS obligations is a continuing covenant of this contract.",
])

page_break(doc)

# =====================================================================
# SECTION 26 — COMPLIANCE DECLARATIONS + SIGNATURE
# =====================================================================
add_heading(doc, "26. Compliance Declarations & Authorised Signature", 1)

add_heading(doc, "26.1 Compliance Declarations", 2)
add_para(doc, "[Vendor Legal Entity Name] confirms the following declarations as a condition of this submission:")
add_bullets(doc, [
    "We have read and understood RFP DLD-IRETP-2026-001 v1.3 in full, including all annexes, and we accept its terms in the manner set out in this proposal.",
    "We confirm no conflict of interest, no undisclosed related-party relationship, and no anti-bribery violation in connection with this procurement.",
    "We confirm UAE PDPL, DESC ISR v3, ISO 27001, WPS, AML, and Anti-Bribery compliance throughout the engagement.",
    "We confirm that the named delivery team in Section 20.4 is committed to the engagement and that replacement requires DLD written approval.",
    "We confirm that all platform IP vests in DLD per Section 25.1 and Section 16.2.",
    "We confirm that all proposal pricing in Section 12 is firm and binding for 120 days from the submission date.",
    "We confirm that the bid bond in Section 20.1 is valid, accessible, and held by [UAE bank] in accordance with RFP §16.",
    "We are not subject to any disqualifying procurement debarment under UAE Federal Law.",
])

add_heading(doc, "26.2 Authorised Signature", 2)
add_para(doc, "Signed and authorised on behalf of [Vendor Legal Entity Name]:", space_before=10, space_after=20)

# Signature block
add_table(doc, ["", "Authorised Signatory", "Witness"], [
    ["Name",         "[Name of Authorised Signatory]",         "[Name of Witness]"],
    ["Position",     "[Title — e.g., Managing Director]",      "[Title]"],
    ["Date",         "18 May 2026",                              "18 May 2026"],
    ["Signature",    "________________________________",        "________________________________"],
    ["Company Seal", "[Vendor Legal Entity Name — official seal]", ""],
], col_widths_in=[1.3, 2.85, 2.85])

add_para(doc, "Submitted with respect to the Dubai Land Department.", italic=True, color=COL_MUTED, space_before=14)
...
# ANNEX F — ACRONYMS & GLOSSARY
# =====================================================================
add_heading(doc, "Annex F — Acronyms & Glossary", 1)
add_para(doc, "Acronyms used in this proposal are listed alphabetically below.")
add_table(doc, ["Acronym / Term", "Definition"], [
    ["AAA",        "Authentication, Authorisation, Audit."],
    ["API",        "Application Programming Interface."],
    ["BAU",        "Business As Usual — the steady-state operating mode post Go-Live."],
    ["BCDR",       "Business Continuity & Disaster Recovery."],
    ["BO",         "Beneficial Ownership — disclosure of the natural person(s) controlling a corporate entity."],
    ["CAPEX",      "Capital Expenditure — one-time investment."],
    ["CMS",        "Content Management System."],
    ["CQRS",       "Command Query Responsibility Segregation — separation of read and write models."],
    ["DDD",        "Domain-Driven Design."],
    ["DESC ISR v3","Dubai Electronic Security Centre — Information Security Regulation, version 3."],
    ["D33",        "Dubai Economic Agenda — the strategic roadmap to 2033."],
    ["DLD",        "Dubai Land Department."],
    ["EF Core",    "Entity Framework Core — Microsoft's object-relational mapper for .NET."],
    ["EWRS",       "Early Warning Risk System — the IRETP risk-detection engine (RFP §8)."],
    ["FX",         "Foreign Exchange (currency conversion)."],
    ["GRETI",      "Global Real Estate Transparency Index — JLL's biennial transparency benchmark."],
    ["GRS",        "Geo-Redundant Storage."],
    ["HMAC",       "Hash-based Message Authentication Code."],
    ["HSM",        "Hardware Security Module."],
    ["IaC",        "Infrastructure as Code (Bicep, Terraform)."],
    ["IRETP",      "Integrated Real Estate Transparency Platform — the subject of this RFP."],
    ["JWT",        "JSON Web Token."],
    ...
    ["NFR",        "Non-Functional Requirement (security, performance, availability, etc.)."],
    ["OIDC",       "OpenID Connect."],
    ["OLAP / OLTP","Online Analytical / Transaction Processing."],
    ["OPEX",       "Operational Expenditure — recurring cost."],
    ["OTLP",       "OpenTelemetry Protocol."],
    ["PaaS",       "Platform as a Service."],
    ["PDPL",       "UAE Personal Data Protection Law (Federal Decree-Law 45 of 2021)."],
    ["PD",         "Person-Day."],
    ["PII",        "Personally Identifiable Information."],
    ["PITR",       "Point-In-Time Restore."],
    ["PoC",        "Proof of Concept."],
    ["PSF",        "Per Square Foot — real-estate pricing unit (AED/sqft)."],
    ["P95 / P90",  "95th / 90th percentile latency."],
    ["RACI",       "Responsible, Accountable, Consulted, Informed."],
    ["RAG",        "Retrieval-Augmented Generation — grounding LLM responses in retrieved factual data."],
    ...
    ["SOP",        "Standard Operating Procedure (linked to EWRS Playbooks)."],
    ...
    ["WCAG",       "Web Content Accessibility Guidelines — IRETP targets WCAG 2.1 AA."],
    ["WPS",        "UAE Wage Protection System."],
    ["YoY",        "Year over Year."],
], col_widths_in=[1.6, 5.4])
